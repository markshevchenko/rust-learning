...
fn main() {
    ...
    let bounds = parse_pair(&args[2], 'x').expect("Unrecognized bounds");
    let upper_left = parse_complex(&args[3]).expect("Unrecognized upper left");
    let lower_right = parse_complex(&args[4]).expect("Unrecognized lower right");
...
//    render(&mut pixels, bounds, upper_left, lower_right);
    ...
    write_image(&args[1], &pixels, bounds).expect("PNG wriging error");
}
...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair::<f64>("8.5x",    'x'), None);
